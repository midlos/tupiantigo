...
// Indigenous-inspired color palettes - 10 Distinct Themes
const CARD_THEMES = [
    {
        id: 'urucum',
        name: 'Urucum',
        border: 'border-l-8 border-red-700', // Darkened from 600 to 700 for consistency
        bgHover: 'hover:bg-red-50',
        badge: 'bg-red-100 text-red-800',
        badgeActive: 'group-hover:bg-red-600 group-hover:text-white',
        textHover: 'group-hover:text-red-900',
    ...
    },
    {
        id: 'genipapo',
        name: 'Genipapo',
        border: 'border-l-8 border-slate-700', // Genipapo turns black/dark blue
        bgHover: 'hover:bg-slate-50',
        badge: 'bg-slate-200 text-slate-800',
        badgeActive: 'group-hover:bg-slate-900 group-hover:text-white',
        textHover: 'group-hover:text-slate-900',
        tag: 'bg-slate-100 text-slate-700',
        decor: 'text-slate-900'
    },
    {
        id: 'acai',
        name: 'Açaí',
        border: 'border-l-8 border-purple-700', // Deep purple
        bgHover: 'hover:bg-purple-50',
        badge: 'bg-purple-100 text-purple-900',
        badgeActive: 'group-hover:bg-purple-800 group-hover:text-white',
        textHover: 'group-hover:text-purple-900',
        tag: 'bg-purple-50 text-purple-800',
        decor: 'text-purple-900'
    },
    {
        id: 'mata',
        name: 'Mata',
        border: 'border-l-8 border-green-700', // Forest
        bgHover: 'hover:bg-green-50',
        badge: 'bg-green-100 text-green-800',
        badgeActive: 'group-hover:bg-green-700 group-hover:text-white',
        textHover: 'group-hover:text-green-900',
        tag: 'bg-green-50 text-green-700',
        decor: 'text-green-900'
    },
    {
        id: 'barro',
        name: 'Barro',
        border: 'border-l-8 border-orange-700', // Clay/Ceramics
        bgHover: 'hover:bg-orange-50',
        badge: 'bg-orange-100 text-orange-800',
        badgeActive: 'group-hover:bg-orange-700 group-hover:text-white',
        textHover: 'group-hover:text-orange-900',
        tag: 'bg-orange-50 text-orange-700',
        decor: 'text-orange-900'
    },
    {
        id: 'agua',
        name: 'Água',
        border: 'border-l-8 border-cyan-700', // Rivers
        bgHover: 'hover:bg-cyan-50',
        badge: 'bg-cyan-100 text-cyan-800',
        badgeActive: 'group-hover:bg-cyan-700 group-hover:text-white',
        textHover: 'group-hover:text-cyan-900',
    ...
    },
    {
        id: 'arara',
        name: 'Arara',
        border: 'border-l-8 border-yellow-700/20', // Darkened from 600 to 700 to match others opacity
        bgHover: 'hover:bg-yellow-50',
        badge: 'bg-yellow-100 text-yellow-800',
        badgeActive: 'group-hover:bg-yellow-600 group-hover:text-white',
        textHover: 'group-hover:text-yellow-900',
        tag: 'bg-yellow-50 text-yellow-700',
        decor: 'text-yellow-900'
    },
    {
        id: 'boto',
        name: 'Boto',
        border: 'border-l-8 border-pink-700', // Darkened from 600 to 700
        bgHover: 'hover:bg-pink-50',
        badge: 'bg-pink-100 text-pink-800',
        badgeActive: 'group-hover:bg-pink-600 group-hover:text-white',
        textHover: 'group-hover:text-pink-900',
    ...
    },
    {
        id: 'cacau',
        name: 'Cacau',
        border: 'border-l-8 border-stone-700/20', // Darkened from 600 to 700 to look more solid
        bgHover: 'hover:bg-stone-50',
        badge: 'bg-stone-200 text-stone-800',
        badgeActive: 'group-hover:bg-stone-700 group-hover:text-white',
        textHover: 'group-hover:text-stone-900',
        tag: 'bg-stone-100 text-stone-700',
        decor: 'text-stone-900'
    },
    {
        id: 'anil',
        name: 'Anil',
        border: 'border-l-8 border-indigo-700', // Deep Indigo Blue
        bgHover: 'hover:bg-indigo-50',
        badge: 'bg-indigo-100 text-indigo-900',
        badgeActive: 'group-hover:bg-indigo-700 group-hover:text-white',
        textHover: 'group-hover:text-indigo-900',
        tag: 'bg-indigo-50 text-indigo-700',
        decor: 'text-indigo-900'
    }
];
                ...
                {LESSONS.map((lesson, index) => {
                    ...
                    return (
                        <button
                            ...
                            className={`group bg-white rounded-3xl p-6 border-stone-200 shadow-sm text-left flex flex-col h-full slide-up relative overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:shadow-xl ${theme.border} ${theme.bgHover}`}
                            style={{ animationDelay: `${0.3 + (index * 0.05)}s` }}
                        >
                            {/* Background Number Decoration */}
                            <div className={`absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity transform group-hover:scale-110 duration-500`}>
                                <span className={`text-7xl font-black ${theme.decor}`}>{lesson.id}</span>
                            </div>
                            
                            <div className="flex items-center gap-3 mb-4 relative z-10">
                                <span className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm transition-all duration-300 shadow-sm ${theme.badge} ${theme.badgeActive}`}>
                                    {lesson.id}
                                </span>
                                {index === 0 && (
                                    <span className="bg-green-600 text-white text-[10px] font-bold px-2 py-1 rounded-full shadow-md tracking-wider">
                                        INÍCIO
                                    </span>
                                )}
                            </div>
                            
                            <h4 className={`font-bold text-stone-800 text-lg mb-2 leading-tight transition-colors ${theme.textHover} relative z-10`}>
                                {lesson.title}
                            </h4>
                            
                            <div className="mt-auto pt-4 relative z-10">
                                {tags.map((tag, i) => (
                                    <span key={i} className={`inline-block text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded border border-transparent group-hover:border-black/5 transition-colors ${theme.tag}`}>
                                        {tag}
                                    </span>
                                ))}
                            </div>
